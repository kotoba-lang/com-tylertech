(ns tylertech.main
  "Kotodama WASM entrypoint for the Tylertech clean-room actor (L4) — Clojure port.
  ...
  facts (`tylertech.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tylertech")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tylertech.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tylertech-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tylertech-compat/ready)
